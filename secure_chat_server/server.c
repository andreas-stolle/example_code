...
static void server_state_free(struct server_state *state) {
    ...
    //    db_close(); //parent does not own connection so commented out, hence no file locking conflicts
    for (i = 0; i < MAX_CHILDREN; i++) {
        close(state->children[i].worker_fd);
    }
}
...
int verify_signature(const char *message, unsigned char *signature, 
                     size_t sig_len, const char *public_key_file) {
    ...
    return (result == 1) ? 0 : -1; // 0 = success, -1 = failure
}
